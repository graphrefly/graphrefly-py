...
# ---------------------------------------------------------------------------
# merge: completes after ALL sources complete (GRAPHREFLY-SPEC §1.3.5 / §2)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Tier 1 operator test matrix — 5 operators × 5 concerns = 25 tests
# ---------------------------------------------------------------------------
...
def test_combine_complete_propagation() -> None:
    """combine completes only after ALL sources complete (spec §1.3.5)."""
    a = state(0)
    b = state(0)
    out = combine(a, b)
    sink: list[Messages] = []
