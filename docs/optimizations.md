...
- **Whole-repo `emit` → `down` audit + `up` / backpressure / `message_tier` sweep (all phases, noted 2026-04-07):**
  ...
  - **PY: DONE (2026-04-07).** Renames: `emit_with_batch` → `down_with_batch`, `_emit_to_sinks` → `_down_to_sinks`, `_emit_auto_value` → `_down_auto_value`, `_emit_partition` → `_down_partition`, `_emit_sequential` → `_down_sequential`, `EmitStrategy` → `DownStrategy`, `emit_line` → `flush_line` (reactive-layout), internal closures renamed. `up()` audit: no asymmetries — backpressure (PAUSE/RESUME) correctly implemented in reduction.py. `message_tier()` audit: already clean for tier-gating; individual type dispatch in ai.py is correct (not tier-gating). `NodeActions.emit()` kept. CQRS `CommandActions.emit()` kept. `_manual_emit_used` / `_manual_emit` kept (reference the kept `actions.emit()` method).
...
| **Non-central time** | Do not schedule periodic work with raw `setTimeout` / `setInterval` / `time.sleep` for graph-aligned sampling. Use `fromTimer` / `from_timer` (or other documented `extra` time sources) and compose reactively. Use `monotonic_ns()` / `monotonicNs()` for event ordering, `wall_clock_ns()` / `wallClockNs()` for attribution. | §5.11 |
| **Hardcoded message type checks** | Do not hardcode `if type == MessageType.DATA` for checkpoint or batch gating. Use `message_tier` / `messageTier` utilities for tier classification. | §5.11 |
