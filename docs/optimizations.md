...
- **~~`equals` must never see undefined/None (all phases, noted 2026-04-05, resolved 2026-04-05):~~** Resolution now uses `_has_emitted_data` flag. Both TS and PY fixed. Test `test_from_any_none_scalar` updated — first DATA(None) correctly treated as changed per spec §2.5.
- **~~`_cached is None` sentinel is ambiguous with `[(DATA, None)]` and INVALIDATE (noted 2026-04-05, resolved 2026-04-05):~~** Resolution: Added `_has_emitted_data: bool` flag (initially `False`, set `True` on first `_emit_auto_value`, reset on INVALIDATE/`reset_on_teardown`). Both TS and PY. Disambiguates "never emitted" from "emitted None" and "reset via INVALIDATE".
...
- **`to_json()` → `to_dict()` + `to_json_string()` (Phase 1.4, noted 2026-04-05):** Current: PY `to_json()` returns a JSON `str`. TS `toJSON()` returns a plain object. **Decision:** rename PY to `to_dict()` (returns `dict`) + `to_json_string()` (returns `str` with trailing newline). TS renames to `toObject()` + `toJSONString()` (already exists). PY `to_json()` becomes alias of `to_json_string()` during transition. Pre-1.0, no backward compat required.
- **Initial value, cached state, and equals interaction (all phases, noted 2026-04-05):** The `initial` option, `_cached` field, `_has_emitted_data` flag, INVALIDATE reset, `reset_on_teardown`, and `equals` function form a tightly coupled state machine with several edge cases: (1) `initial` sets `_cached` but `_has_emitted_data` stays `False` — first `_emit_auto_value` skips `equals` even if value matches `initial`; is this correct? (2) After INVALIDATE, `_cached = None` and `_has_emitted_data = False` — next recompute always emits DATA even if value is the same as pre-invalidation. (3) `actions.emit(None)` sets `_cached = None` — subsequent emissions with `_has_emitted_data = True` call `equals(None, new_value)` which is fine for default `is` but custom `equals` might not expect `None`. (4) Could a `_SENTINEL` object replace both `_has_emitted_data` and the `initial` default, unifying the state machine? (5) **Resubscribable nodes:** after terminal, `_has_emitted_data` stays `True`. On resubscribe, first emission matching `_cached` emits RESOLVED — new subscriber never sees DATA. Should `_has_emitted_data` reset alongside `_terminal = False` in the resubscribe path? (6) **Reconnect (disconnect/reconnect):** all sinks leave → new sink arrives → `_has_emitted_data` is `True` → same value emits RESOLVED. Arguably correct (cache valid, `get()` returns it) but new subscriber's first signal is RESOLVED without prior DATA. Deferred: discuss systematically in next chat — may eliminate `_has_emitted_data` entirely.
