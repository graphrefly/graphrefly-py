---
name: parity
description: "Cross-language parity check + adversarial QA pass across graphrefly-py and graphrefly-ts. Run after /dev-dispatch + /qa on both repos. Use when user says 'parity', 'cross-lang check', or 'sync repos'."
disable-model-invocation: true
argument-hint: "[feature area or 'full'] [optional: path to sibling repo]"
---

You are executing the **parity** workflow, comparing **graphrefly-py** (this repo) against **graphrefly-ts** (`~/src/graphrefly-ts` unless overridden in $ARGUMENTS).
...
|--------|--------|-----------|---------|
| ... | ... | ... | aligned / Py ahead / TS ahead / intentional divergence |
...
1. Apply fixes to **this repo** (graphrefly-py) — code + tests
2. Run `uv run pytest` — fix failures
